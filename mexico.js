...
document.addEventListener('DOMContentLoaded', function() {
  ...
  botoesSortear.forEach(function(botao) {
    botao.addEventListener('click', async function() {
      ...
      async function sortearTris(jogoId) {
  const container = document.querySelector('#' + jogoId + ' .resultado-bolas');
  container.innerHTML = '';

  const numeros = [];
  for (let i = 0; i < 5; i++) {
    const digito = Math.floor(Math.random() * 10); // de 0 a 9
    numeros.push(digito);
  }

  for (let i = 0; i < numeros.length; i++) {
    await criarBolaComAnimacao(container, numeros[i]);
    await esperar(700);
  }
}
  ...
  switch (idJogo) {
  case 'melate':
    // 6 números (1–56) + 1 adicional (1–56)
    await sortearNumeros(idJogo, 6, 56, 1, 56);
    break;

  case 'melate-retro':
    // 6 números (1–39) + 1 adicional (1–39)
    await sortearNumeros(idJogo, 6, 39, 1, 39);
    break;

  case 'melate-revancha':
    // 6 números (1–56) sem adicional
    await sortearNumeros(idJogo, 6, 56);
    break;

  case 'melate-revanchita':
    // 6 números (1–56) sem adicional
    await sortearNumeros(idJogo, 6, 56);
    break;
  ...
  case 'chispazo':
    // 5 números (1–28)
    await sortearNumeros(idJogo, 5, 28);
    break;

 case 'tris':
  await sortearTris(idJogo);
  break;
