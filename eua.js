...
document.addEventListener('DOMContentLoaded', function() {
  ...
  switch (idJogo) {
  case 'powerball':
    await sortearNumeros(idJogo, 5, 69, 1, 26); // ✅ 5 números (1–69) + 1 Powerball (1–26)
    break;

  case 'mega-millions':
    await sortearNumeros(idJogo, 5, 70, 1, 25); // ✅ 5 números (1–70) + 1 Mega Ball (1–25)
    break;

  case 'ny-lotto':
  await sortearNumeros(idJogo, 6, 59, 1, 59);
  break;


  case 'cash4life-ny':
    await sortearNumeros(idJogo, 5, 60, 1, 4); // ✅ 5 números (1–60) + 1 Cash Ball (1–4)
    break;

  case 'take-5':
    await sortearNumeros(idJogo, 5, 39); // ✅ 5 números (1–39) — sem extra
    break;

  case 'pick-10':
    await sortearNumeros(idJogo, 10, 80); // ✅ Jogador escolhe 10 de 1 a 80 — Keno-style
    break;

  case 'win-4-ny':
    await sortearNumeros(idJogo, 4, 9); // ✅ 4 dígitos de 0–9 — estilo Pick 4
    break;

  case 'numbers-ny':
    await sortearNumeros(idJogo, 3, 9); // ✅ 3 dígitos de 0–9 — estilo Pick 3
    break;

  default:
    await sortearNumeros(idJogo, 6, 49); // Fallback padrão
}
