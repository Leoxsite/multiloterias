...
document.addEventListener('DOMContentLoaded', function() {
  ...
  case 'oz-lotto':
    // Oz Lotto: 7 números + 3 suplementares (1‑47)
    await sortearNumeros(idJogo, 7, 47, 3, 47);
    break;

  case 'powerball-au':
    // Powerball AU: 7 números (1‑35) + 1 Powerball (1‑20)
    await sortearNumeros(idJogo, 7, 35, 1, 20);
    break;

  case 'saturday-lotto':
    // Saturday Lotto: 6 números + 2 suplementares (1‑45)
    await sortearNumeros(idJogo, 6, 45, 2, 45);
    break;

  case 'super66':
    // Super 66: 6 dígitos fixos estilo pick‑6 de 0‑9
    await sortearNumeros(idJogo, 6, 10);
    break;

  case 'lotto-strike':
    // Lotto Strike (Strike Four): 4 dígitos fixos estilo posicional
    await sortearNumeros(idJogo, 4, 10);
    break;

  case 'set-for-life':
    // Set for Life: 7 números + 2 suplementares (1‑44)
    await sortearNumeros(idJogo, 7, 44, 2, 44);
    break;

  case 'cash-3':
    // Cash 3: 3 dígitos estilo pick‑3 (0‑9)
    await sortearNumeros(idJogo, 3, 10);
    break;

  case 'monday-wednesday-lotto':
    // Monday & Wednesday Lotto: 6 números + 2 suplementares (1‑45)
    await sortearNumeros(idJogo, 6, 45, 2, 45);
    break;

  case 'weekday-windfall':
  // Weekday Windfall: 6 números principais + 2 suplementares (1–45)
  await sortearNumeros(idJogo, 6, 45, 2, 45);
  break;
  ...
  default:
    // fallback padrão, caso seja um jogo não listado
