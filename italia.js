...
async function sortearNumeros(jogoId, quantidadePrincipal, maximoPrincipal, quantidadeExtra, maximoExtra, usarDigitoSimples, incluirZero) {
  ...
  if (numerosExtras.length > 0) {
    ...
    for (let i = 0; i < numerosExtras.length; i++) {
      const dourada = (jogoId === 'eurojackpot-it' && i >= numerosExtras.length - 2);
...
document.addEventListener('DOMContentLoaded', function() {
  ...
  case 'eurojackpot-it':
    await sortearNumeros(idJogo, 5, 50, 2, 12); // ✅ 5 números (1–50) + 2 EuroNumbers (1–12)
    break;

  case 'superenalotto':
    await sortearNumeros(idJogo, 6, 90); // ✅ 6 números (1–90) — sem bola extra
    break;

  case 'n10elotto':
    await sortearNumeros(idJogo, 10, 90); // ✅ 10 números (1–90) — estilo Keno
    break;

  case 'millionday':
    await sortearNumeros(idJogo, 5, 55); // ✅ 5 números (1–55) — sem extra
    break;

  case 'vincicasa':
    await sortearNumeros(idJogo, 5, 40); // ✅ 5 números (1–40) — sem extra
    break;

  case 'lottomatica':
    await sortearNumeros(idJogo, 5, 90); // ✅ 5 números (1–90) — sem extra
    break;

  case 'sivincetutto':
    await sortearNumeros(idJogo, 12, 90); // ✅ 6 números (1–90) — sem extra
    break;
