...
document.addEventListener('DOMContentLoaded', function() {
  ...
  case 'euromillions':
    // 5 números (1–50) + 2 estrelas (1–12)
    await sortearNumeros(idJogo, 5, 50, 2, 12);
    break;

  case 'lotto':
    // 6 números (1–59) sem adicional
    await sortearNumeros(idJogo, 6, 59);
    break;

  case 'all-or-nothing':
    // 12 números (1–24) sem adicional
    await sortearNumeros(idJogo, 12, 24);
    break;

  case 'n49s':
    // 6 números (1–49) + 1 adicional sorteado (booster)
    await sortearNumeros(idJogo, 6, 49);
    break;

  case 'lotto-hotpicks':
  // 6 números (1–59), sem adicionais
  await sortearNumeros(idJogo, 6, 59);
  break;


  case 'health-lottery':
  // 5 números (1–50) + 1 bônus (1–50)
  await sortearNumeros(idJogo, 5, 50, 1, 50);
  break;


  case 'thunderball':
    // 5 números (1–39) + 1 thunderball (1–14)
    await sortearNumeros(idJogo, 5, 39, 1, 14);
    break;

  case 'set-for-life':
    // 5 números (1–47) + 1 life ball (1–10)
    await sortearNumeros(idJogo, 5, 47, 1, 10);
    break;


  case 'pick-3-uk':
    // Pick 3: 3 números entre 0 e 9, com repetição
    {
      const container = document.querySelector('#' + idJogo + ' .resultado-bolas');
      container.innerHTML = '';

      for (let i = 0; i < 3; i++) {
        const num = Math.floor(Math.random() * 10); // 0 a 9
        await criarBolaComAnimacao(container, num);
        await esperar(700);
      }
    }
    break;
