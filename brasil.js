...
document.addEventListener('DOMContentLoaded', function() {
  ...
  botoesSortear.forEach(function(botao) {
    botao.addEventListener('click', async function() {
      ...
      switch(idJogo) {
        case 'mega-sena':
          await sortearNumeros(idJogo, 6, 60);
          break;

        case 'lotofacil':
          await sortearLotofacil(idJogo);
          break;

        case 'quina':
          await sortearNumeros(idJogo, 5, 80);
          break;

        case 'lotomania':
          await sortearNumeros(idJogo, 50, 99, 0, 0, false, true);
          break;

        case 'timemania':
          await sortearNumeros(idJogo, 10, 80);
          break;

        case 'dupla-sena':
          await sortearDuplaSena(idJogo);
          break;

        case 'dia-de-sorte':
         // Dia de Sorte: 7 números (1‑31) + 1 mês de sorte (1‑12)
         await sortearNumeros(idJogo, 7, 31, 1, 12);
         break;
        

        case 'super-sete':
          await sortearNumeros(idJogo, 7, 10);
          break;

        case 'milionaria':
          await sortearNumeros(idJogo, 6, 50, 2, 6);
          break;


        default:
          console.warn('Jogo não configurado:', idJogo);
      }
